...
export function OpponentSelectCombobox({ clubs }: { clubs: Club[] }) {
    const [selectedClub, setSelectedClub] = useQueryState('opponent', {
        defaultValue: '',
    });
            ...
            <Popover>
                <PopoverTrigger asChild>
                    <Button
                        variant="outline"
                        role="combobox"
                        className={cn('w-[300px] justify-between', !selectedClub && 'text-muted-foreground')}
                    >
                        {selectedClub ? (
